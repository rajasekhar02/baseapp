...
Description: This is the modified version of the element ui date utils to change the functionality of the date picker according to pondlogs requirements
*/
...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => i18n.t(`el.datepicker.weeks.${week}`)),
    dayNames: weeks.map(week => i18n.t(`el.datepicker.weeks.${week}`)),
    monthNamesShort: months.map(month =>
      i18n.t(`el.datepicker.months.${month}`)
    ),
    monthNames: months.map((month, index) =>
      i18n.t(`el.datepicker.month${index + 1}`)
    ),
    amPm: ["am", "pm"]
  };
};
...
export const getRangeHours = function() {
  const hours = [];
  const disabledHours = [];
...
export const getRangeMinutes = function() {
...
export const limitTimeRange = function(date, ranges) {
...
export const extractTimeFormat = function(format) {
  return format
    .replace(/\W?D{1,2}|\W?Do|\W?d{1,4}|\W?M{1,4}?(\W+)|\W?y{2,4}\W+/g, "")
    .trim();
};
